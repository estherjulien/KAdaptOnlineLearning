...
from ShortestPath.functions import *
...
def algorithm(K, env, time_limit=20*60, print_info=True, problem_type="test"):
    ...
    while N_set and time.time() - start_time < time_limit:
        ...
        if time.time() - start_time - prev_save_time > 10*60:
            ...
            tmp_results = {"theta": theta_i, "x": x_i, "y": y_i, "tau": tau_i, "inc_thetas_t": inc_thetas_t,
                           ...
                           "runtime": time.time() - start_time, "tot_nodes": inc_tot_nodes,
                           "num_nodes_curr": inc_tot_nodes, "mp_time": mp_time, "sp_time": sp_time}
            with open(f"ClusterOnlineLearning/Results/SP/Decisions/tmp_results_{problem_type}_inst{env.inst_num}.pickle", "wb") as handle:
    ...
    results = {"theta": theta_i, "x": x_i, "y": y_i, "tau": tau_i, "inc_thetas_t": inc_thetas_t,
               "inc_thetas_n": inc_thetas_n, "inc_x": inc_x, "inc_y": inc_y, "inc_tau": inc_tau,
               "runtime": time.time() - start_time, "inc_tot_nodes": inc_tot_nodes,
               "tot_nodes": tot_nodes, "mp_time": mp_time, "sp_time": sp_time, "scen_all": scen_all}

    with open(f"ClusterOnlineLearning/Results/SP/Decisions/final_results_{problem_type}_inst{env.inst_num}.pickle", "wb") as handle:
