...
def algorithm(K, env, att_series, success_model_name=None, time_limit=20 * 60, print_info=True,
    ...
    # initialize N_set
    theta_i, x_i, y_i, tau_i, N_set, scen_all, att_all, \
    ...
    if success_model_name is None:
        success_model_name = f"Results/NNModels/nn_model_alt_cb_online_learning_sub_tree_K2_N10_D1_W50_inst0.h5"

    success_model = load_model(success_model_name)
    _, att_index, _ = init_weights_fun(K, env, att_series)
    ...
    while N_set and time.time() - start_time < time_limit:
        ...
        if len(full_list) == 0:
            ...
            tau_s = state_features(K, env, theta, zeta, x, y, tot_scens, init_tot_scens, tau_att, theta_i, zeta_init,
                                   att_index)
