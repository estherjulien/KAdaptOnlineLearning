...
def predict_subset(K, tau_att, scen_att, scen_att_k, success_model, att_index, state_features, normalize_values=None):
    X = input_fun(K, state_features, tau_att, scen_att, scen_att_k, att_index)
    if normalize_values is not None:
        X_new = X / normalize_values
        X = np.nan_to_num(X_new, nan=0)
...
def update_model_fun(X, Y, expert_data_num=0, depth=1, width=10, success_model_name="test", normalize=None):
