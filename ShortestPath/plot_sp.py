...
for i in np.arange(num_inst):
    ...
    with open(f"Results/Decisions/tmp_results_sp_obj_rule_K4_N100_g30_fs30_inst{i}.pickle", "rb") as handle:
        results_r1[i] = pickle.load(handle)[1]
...
for i in np.arange(num_inst):
    ...
    t_r1 = np.zeros(num_grids)
    for t, theta in results_r1[i]["inc_thetas_t"].items():
        t_r1[t_grid > t] = theta/theta_final
    obj_r1[f"r1_{i}"] = t_r1
