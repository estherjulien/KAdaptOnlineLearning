...
from EuclDist.OnlineLearning import algorithm as algorithm_o
from EuclDist.Strategy import algorithm as algorithm_s
...
for N in [30, 40, 50]:
    for K in [3, 4]:
        ...
        for rf_num in np.arange(3):
            ...
            problem_type = f"sp_eucl_att_K{K}_N{N}_rf{rf_num}"
        ...
        for rf_num in np.arange(3):
            problem_type = f"sp_eucl_att_strategy_K{K}_N{N}_rf{rf_num}"
            success_model_name = f"ResultsEuclAtt/RFModels/rf_model_sp_suc_pred_K{K}_N{N}_rf{rf_num}.joblib"
