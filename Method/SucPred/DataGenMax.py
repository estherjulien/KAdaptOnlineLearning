...
def data_gen_fun_max(K, env, att_series, n_back_track=2, time_limit=20 * 60, problem_type="test",
    ...
    while N_set and time.time() - start_time < time_limit:
        ...
        if time.time() - start_time - prev_save_time > 10 * 60:
            ...
            with open(f"Data/ResultsSucPred/Data/tmp_results_{problem_type}_rf{env.inst_num}.pickle", "wb") as handle:
    ...
    with open(f"Data/ResultsSucPred/Data/final_results_{problem_type}_rf{env.inst_num}.pickle", "wb") as handle:
...
def data_and_train_max(K, env_list, att_series, n_back_track=2, time_limit=20 * 60, problem_type="test",
                       thread_count=8, max_depth=5, width=50, depth=1, train_on=None, init_data=False):
    # DATA GEN
    if init_data:
        with open(f"Data/ResultsSucPred/Data/all_data_{problem_type}_10.pickle", "rb") as handle:
    ...
    results = []
    for env in env_list:
        results.append(data_gen_fun_max(K, env, att_series, n_back_track, time_limit, problem_type,
