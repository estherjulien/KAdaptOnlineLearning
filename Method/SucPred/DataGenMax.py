...
def data_gen_fun_max(K, env, att_series, n_back_track=2, time_limit=20 * 60, problem_type="test",
    ...
    while N_set and time.time() - start_time < time_limit:
        ...
        if time.time() - start_time - prev_save_time > 10 * 60:
            ...
            with open(f"Data/ResultsSucPred/Data/inst_results/tmp_results_{problem_type}_rf{env.inst_num}.pickle", "wb") as handle:
...
def data_and_train_max(K, env_list, att_series, n_back_track=2, time_limit=20 * 60, problem_type="test",
    ...
    for env in env_list:
        if env.inst_num != 25:
            continue
        results = data_gen_fun_max(K, env, att_series, n_back_track, time_limit, problem_type,
    ...
    for i in np.arange(num_envs):
        with open(f"Data/ResultsSucPred/Data/inst_results/data_{problem_type}_{i}.pickle", "rb") as handle:
