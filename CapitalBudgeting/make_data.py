...
if __name__ == "__main__":
    ...
    perc_label = float(sys.argv[5])
    normalized = float(sys.argv[6])
    ...
    if normalized:
        problem_type = f"cb_norm_p{int(perc_label*100)}_N{N}_K{K}"
    else:
        problem_type = f"cb_p{int(perc_label*100)}_N{N}_K{K}"
