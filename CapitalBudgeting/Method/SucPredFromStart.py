...
def algorithm(K, env, att_series=None, max_level=5, success_model_name=None, time_limit=30 * 60, print_info=True,
    ...
    N_set_trash = []
    # run a few times to get theta_init, tot_scens_init and zeta_init
    ...
    while (N_set or N_set_trash) and time.time() - start_time < time_limit:
        ...
        # if len(full_list) == K:
        #     # predict subset
        if sum([len(t) for t in tau.values()]) >= max_level:
            K_set = np.arange(K)
            ...
            tau_att = {k: np.hstack([att_all[placement[k]], k_att_sel[k]]) for k in full_list}
            tau_s = state_features(theta, zeta, tot_scens, tot_scens_init, theta_init, zeta_init, theta_pre, zeta_pre)
            K_set, predictions = predict_subset(K, tau_att, scen_att, scen_att_k, success_model, att_index, tau_s, full_list)
...
def fill_subsets(K, env, att_series, x_static, progress=False):
    ...
    N_set_init = [{k: [] for k in np.arange(K)}]
    N_set_init[0][0].append(0)
    N_set = copy.deepcopy(N_set_init)
    scen_all_init = xi_init.reshape([1, -1])
    scen_all = copy.deepcopy(scen_all_init)
    att_all_init = np.array(att_init).reshape([1, -1])
    att_all_k_init = [att_init_k]
    new_xi_num = 0
    while True:
        # MASTER PROBLEM
        if new_model:
            ...
            tau_init = {k: scen_all[placement[k]] for k in np.arange(K)}
            tau = copy.deepcopy(tau_init)
            theta, x, y, model = scenario_fun_build(K, tau, env)
        else:
            # NEW NODE from k_new
            # master problem
    ...
    return N_set_init, tau_init, scen_all_init, att_all_init, att_all_k_init, zeta
