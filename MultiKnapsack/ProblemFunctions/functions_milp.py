...
def scenario_fun_update(K, k_new, xi_new, env, scen_model):
    ...
    scen_model.addConstr((gp.quicksum(env.cost[i]*(1 - 0.1*xi_new[i])*y[k_new][i] for i in np.arange(env.N))) >= theta)
...
def scenario_fun_build(K, tau, env):
    ...
    for k in np.arange(K):
        for xi in tau[k]:
            # objective constraint
            scen_model.addConstr(gp.quicksum(env.cost[i]*(1 - 0.1*xi[i])*y[k][i] for i in np.arange(env.N)) >= theta)
...
def separation_fun(K, x, y, theta, env, tau):
    ...
    for k in np.arange(K):
        if len(tau[k]) > 0:
            sep_model.addConstr(zeta <= -(gp.quicksum(env.cost[i]*(1 - 0.1*xi[i])*y[k][i] for i in np.arange(env.N)) -
                                theta))
    # uncertainty set
...
def scenario_fun_update_sub_tree(K, new_node, xi_dict, env, scen_model=None):
    ...
    for node_sec in np.arange(1, len(new_node)):
        ...
        # add new constraints
        # objective constraint
        scen_model.addConstr(gp.quicksum(env.cost[i]*(1 - 0.1*xi_new[i])*y[k_new][i] for i in np.arange(env.N))
...
def scenario_fun_deterministic_update(env, scen, smd):
    ...
    # constraints
    # objective constraint
    smd.addConstr(gp.quicksum(env.cost[i] * (1 - 0.1 * scen[i]) * y[i] for i in np.arange(env.N)) >= theta, name="new_const_1")
