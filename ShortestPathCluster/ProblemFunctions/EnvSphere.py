...
class Graph:
    ...
    def plot_graph(self, name=None):
        ...
        plt.axis("off")
        if name is None:
            plt.savefig(f"ShortestPathCluster/Data/Plots/graph_N{self.N}_tap{self.throw_away_perc}_d{self.degree}_inst{self.inst_num}")
        else:
            plt.savefig(f"ShortestPathCluster/Data/Plots/graph_{name}_N{self.N}_tap{self.throw_away_perc}_d{self.degree}_inst{self.inst_num}")
