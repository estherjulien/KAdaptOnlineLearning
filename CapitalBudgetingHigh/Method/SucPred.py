...
def algorithm(K, env, att_series=None, max_level=5, success_model_name=None, time_limit=30 * 60, print_info=True,
    ...
    theta_init, theta_i, tot_scens_init, tot_nodes = init_random_passes(K, env, tau)
...
def init_random_passes(K, env, tau, num_runs=5):
    init_results = []

    for i in np.arange(num_runs):
        init_results.append(random_pass(K, env, tau))

    theta_init = np.mean([res[0] for res in init_results])
    theta_i = np.min([res[0] for res in init_results])
    tot_scens_init = np.mean([res[1] for res in init_results])
    nodes_used = np.sum([res[2] for res in init_results])
    return theta_init, theta_i, tot_scens_init, nodes_used
