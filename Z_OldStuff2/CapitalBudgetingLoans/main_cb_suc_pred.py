from Z_OldStuff2.Method.SucPred.Strategy import algorithm as algorithm_s
...
for N in [10]:
    for K in [3, 4]:
        ...
        for comb in train_combinations:
            problem_type = f"cb_suc_pred_ML[K{K_train}_N{N_train}_rf{comb}]_T[K{K}_N{N}]"
            success_model_name = f"Data/ResultsSucPred/RFModels/rf_model_cb_suc_pred_data_K{K_train}_N{N_train}_rf{comb}.joblib"
            ...
            results = dict()
            for i in np.arange(num_instances):
                with open(f"Data/ResultsSucPred/Decisions/inst_results/final_results_{problem_type}_inst{i}.pickle", "rb") \
                        as handle:
                    results[i] = pickle.load(handle)
            with open(f"Data/ResultsSucPred/Decisions/FINAL_results_{problem_type}_{num_instances}.pickle", "wb") as handle:
