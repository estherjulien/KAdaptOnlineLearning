...
from CapitalBudgetingLoans.functions import *
...
def algorithm(K, env, time_limit=20*60, print_info=True, problem_type="test"):
    ...
    while N_set and time.time() - start_time < time_limit:
        ...
        if time.time() - start_time - prev_save_time > 10*60:
            ...
            with open(f"ClusterOnlineLearning/Results/CB/Decisions/tmp_results_{problem_type}_inst{env.inst_num}.pickle", "wb") as handle:
    ...
    with open(f"ClusterOnlineLearning/Results/CB/Decisions/final_results_{problem_type}_inst{env.inst_num}.pickle", "wb") as handle:
